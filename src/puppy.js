...
function buildQueryURLOFF(text, sl, tl) {
  let url = `${URL}&sl=${sl}&tl=${tl}&text=`;
  return text.reduce(
    (url, t) => (url += `${encodeURIComponent(t)}${SEPARATOR_QUERY}`),
    url
  );
}
...
function myencodeURIComponent(text) {
  return encodeURIComponent(
    text.replace(new RegExp("%", "g"), PERCENT_REPLACE)
  );
}
...
function buildQueryURL(text, sl, tl) {
  ...
  for (let t of text) {
    ...
    if (count + len < CHARACTER_LIMIT) {
      querys[idx] = querys[idx] += `${myencodeURIComponent(
        t
      )}${SEPARATOR_QUERY}`;
      count += len;
    } else {
...
async function translate(text, slang = "en", tlang = "es") {
  const proxy = JSON.parse(process.env.USE_PROXY)
    ? await require("./proxy")()
    : "";

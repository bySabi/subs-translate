...
function googleAPIkeySetupValidation(key) {
  if (key) {
    ...
    invariant(
      existsFileSync(key),
      `Provide "--key" file "${key}" cannot be found`
    );
  ...
  PROJECT_ID = require(path.resolve(process.env.GOOGLE_APPLICATION_CREDENTIALS))
    .project_id;
  invariant(
    PROJECT_ID,
    `Invalid JSON key file ${process.env.GOOGLE_APPLICATION_CREDENTIALS}`
  );
}

async function translator(text, slang, tlang) {
...
async function translate(text, slang = "en", tlang = "es") {
  const trans = await translator(text, slang, tlang);
  return trans;
}
